...
export function calculateDocumentPrices(
  shop: Shop | null,
  docs: DocumentFile[],
  fallbackConfig: PrintConfig,
) {
  return docs.map((document) =>
    calculatePrice(shop, [document], document.printConfig ?? fallbackConfig, "pickup"),
  );
}
...
export function calculatePrice(
  ...
  const totalPages = docs.reduce(
    (sum, document) => sum + Math.max(1, Math.floor(document.pages)),
    0,
  );
  const selectedPages =
    config.pageRangeMode === "custom" ? countRangePages(config.pageRange, totalPages) : totalPages;
  if (selectedPages === 0) return empty;
  const selected =
    config.pageRangeMode === "custom" ? selectedPageNumbers(config.pageRange, totalPages) : null;
  let offset = 0;
  const selectedDocuments = docs.reduce((count, document) => {
    const pages = Math.max(1, Math.floor(document.pages));
    const isIncluded = selected
      ? [...selected].some((page) => page > offset && page <= offset + pages)
      : true;
    offset += pages;
  ...
  const binding = money(
    (shop.binding.find((item) => item.id === config.bindingId && item.enabled)?.price ?? 0) *
      documentSets,
  );
  const services = money(
    config.additionalIds.reduce((sum, id) => {
