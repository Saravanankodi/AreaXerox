...
function ShopOrderDetail() {
  ...
  const paymentSatisfied = isFullyPaid || !hasBalance;
          ...
          <div className="card-surface p-5">
            ...
            <div className="mt-4 space-y-2">
              {flow.map((status, i) => {
                ...
                const isCompleted = order.status !== "REJECTED" && i < currentIndex;
                const isCurrent = order.status !== "REJECTED" && i === currentIndex;
                ...
                if (isCurrent && order.status !== "COMPLETED" && order.status !== "DELIVERED") {
                  const actionDisabled = !paymentSatisfied;
                  return (
                    <button
                      key={status}
                      type="button"
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          paymentSatisfied ? "bg-primary" : "bg-primary/30",
                        )}
                      >
                        <span
                          className={cn(
                            "absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white shadow-sm transition-transform",
                            paymentSatisfied && "translate-x-0",
                          )}
                        />
                      </span>
                      {label}
