...
export function ShopShell({
  ...
  useEffect(() => {
    ...
    if (session.registrationStatus === "incomplete") {
      navigate({ to: "/auth/shop/register", replace: true });
    } else if (session.accountStatus === "pending") {
      navigate({ to: "/shop/pending", replace: true });
    } else if (session.accountStatus === "rejected") {
      navigate({ to: "/shop/rejected", replace: true });
    } else if (session.accountStatus === "suspended") {
      navigate({ to: "/shop/suspended", replace: true });
    } else if (session.accountStatus === "disabled") {
      navigate({ to: "/shop/disabled", replace: true });
    }
  }, [session, navigate]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center p-5">
        <div className="card-surface max-w-md p-7 text-center">
          <h1 className="text-xl font-bold">Shopkeeper sign-in required</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Use your shop account to access orders, services and settings.
          </p>
          <Link
            to="/auth/shop/login"
  ...
  if (session.registrationStatus === "incomplete") {
    return (
      <div className="flex min-h-screen items-center justify-center p-5">
        <div className="card-surface max-w-md p-7 text-center">
          <h1 className="text-xl font-bold">Registration incomplete</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Complete your shop registration to access the portal.
          </p>
          <Link
    ...
    return (
      <div className="flex min-h-screen items-center justify-center p-5">
        <div className="card-surface max-w-md p-7 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Your account status does not allow portal access.
          </p>
          <Link
            to="/"
            className="mt-5 inline-block rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground"
    ...
    <div className="min-h-screen bg-background lg:flex">
      <aside className="border-b border-sidebar-border bg-sidebar lg:sticky lg:top-0 lg:h-screen lg:w-64 lg:shrink-0 lg:border-e lg:border-b-0">
        ...
        <div className="hidden px-3 pb-6 lg:block">
          ...
          <button
            onClick={signOut}
            className="mt-1 flex w-full items-center gap-2.5 rounded-md px-3 py-2.5 text-sm font-medium text-muted-foreground hover:bg-secondary hover:text-foreground"
          >
