...
export function AuthPage({ role, mode }: { role: AccountRole; mode: "login" | "signup" }) {
  ...
  const destination = isShop ? "/shop" : orderDraft ? "/order" : "/";
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (isCreate && (!name.trim() || (isShop && !shopName.trim()))) {
      toast.error(
        isShop ? "Add your name and shop name to continue." : "Add your full name to continue.",
      );
      return;
    }
    signIn({
    ...
    toast.success(isCreate ? "Account created" : "Welcome back", {
      ...
      <section className="flex items-center justify-center px-5 py-10 sm:px-8">
        <div className="w-full max-w-md">
          ...
          <form className="mt-7 space-y-4" onSubmit={submit}>
            {isCreate && (
              <Field
                label={isShop ? "Owner name" : "Full name"}
                value={name}
                onChange={setName}
                autoComplete="name"
              />
